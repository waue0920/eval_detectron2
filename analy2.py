...
def mark_bbox_on_images(count_dict, bb_dict, img_list, dataset_path, output_dir):
    ...
    for img_name in img_list:
        ...
        for fn_bb in bb_dict[img_name]['fn']:
            ...
            text = ""
            if isinstance(fn_bb, list) and len(fn_bb) >= 5:
                label = fn_bb[0]
                coords = fn_bb[1:5]
                text = f"{label} (FN)"
...
def run_score_matrix_mode(input_dir, output_dir):
    ...
    with open(save_path, 'w', newline='') as f:
        ...
        writer.writerow(['每張圖片分數計算方式： tp_num*100 - fn_num*1 - fp_num*0'])
        writer.writerows(matrix)
    print(f"分數矩陣已儲存至: {save_path}")
def main():
    parser = argparse.ArgumentParser(description="Stage 2: Qualitative analysis script.")
    parser.add_argument('--input-dir', type=str, default='./eval_1_draw_bbox', help='存放第一階段評估結果的主資料夾路徑。')
    parser.add_argument('--output-dir', type=str, default='./eval_2_result', help='儲存所有分析結果的主資料夾路徑。')
    parser.add_argument('--dataset-path', type=str, help='存放原始 JPG 圖片的資料夾路徑 (在 auto 和 image mode 中為必要)。')
    parser.add_argument('--mode', type=str, choices=['auto', 'image', 'score'], required=True, help='執行模式。')
    parser.add_argument('--image-name', type=str, help='在 image mode 中要處理的圖片檔案名稱 (例如: "munster_000011_000019_leftImg8bit.jpg")。')
